...
public class Library {
    ...
    public List<String> findTitlesByAuthor(String authorName) {
        return books.stream().filter(books->books.getAuthor().equals(authorName))
                .map(Book::getTitle)
                .collect(toList());
    ...
    public List<String> findTitlesOfLongBooks(int minPageCount) {
        ArrayList<String> minpagesbooks = new ArrayList<String>();
        for(Book book: books){
            if(book.getPageCount() > minPageCount){
                minpagesbooks.add(book.getTitle());
            }
        }
        return minpagesbooks;

    }


    public List<String> findTitlesAndAuthorsInGenre(String genre) {
      return  books.stream().filter(books->books.getGenres().contains(genre))
                .map(books->books.getTitle()+ " by " +
                            books.getAuthor())
                .collect(toList());
    }
